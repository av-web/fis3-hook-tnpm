...
module.exports = function(id, opts) {
    ...
    opts.paths.every(function(item) {
        ...
        } else if(item.type === 'tnpm') {
            ret = getTnpm(item.location, id);
        } else {
 ...
 */
function getTnpm(root, id) {
    var tnpm = fis.project.getProjectPath(root)

    // 不处理多级目录
    // 快速判断
    var pkgName = id.replace(/@tencent\//ig, '')

    if (!pkgName || !fs.existsSync(path.join(tnpm, pkgName)) ) {
        return null;
    }

    var subFile = require(_(tnpm, pkgName, 'package.json')).main

    if (!subFile) {
        subFile = 'index.js'
    }

    if (!_.isFile(_(tnpm, pkgName, subFile))) {
        return fis.log.error('tnpm: 没有找到入口文件', tnpm, pkgName, subFile);
    }

    return fis.uri(_(root, pkgName, subFile));
}
